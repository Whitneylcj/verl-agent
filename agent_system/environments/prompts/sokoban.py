...
_SOKOBAN_GUIDED_PLANNING = """# Planning Checklist
Locate the player, every box, and every target before choosing an action.
Choose the next useful box push, then move the player to the side opposite that push direction.
If the previous action left the board unchanged, choose a different non-blocked action instead of repeating a loop.
...
"""
_SOKOBAN_GUIDED_GRID_RESPONSE = """Read the grid literally and do not invent boxes or targets. Number rows top-to-bottom and columns left-to-right.
Check the push geometry internally, then reply in exactly two lines without listing alternatives or restating the grid.
Line 1 must be at most 35 words: Plan: P=(row,column); X=(row,column); O=(row,column); push=DIRECTION; required player cell=(row,column); move=DIRECTION.
Replace every placeholder with actual coordinates or one of up, down, left, right.
Line 2 must be exactly one of: <action>up</action>, <action>down</action>, <action>left</action>, <action>right</action>. Never use <up>, <down>, <left>, or <right> as tags."""
_SOKOBAN_GUIDED_IMAGE_RESPONSE = _SOKOBAN_GUIDED_GRID_RESPONSE.replace(
    "Read the grid literally", "Read the image literally"
)
_SOKOBAN_BENCHMARK_RESPONSE = (
    "You should first reason step-by-step about the current situation — observe the positions of boxes and targets, "
    "plan a path to push a box toward a target, and avoid traps like corners or walls. This reasoning process MUST "
...
def _sokoban_benchmark_prompt(prompt, guided_response):
    return (
        prompt.replace(_SOKOBAN_GUIDED_PLANNING, "")
        .replace("{loop_warning}\n", "")
        .replace(guided_response, _SOKOBAN_BENCHMARK_RESPONSE)
    )


SOKOBAN_TEMPLATE_NO_HIS = _sokoban_benchmark_prompt(
    SOKOBAN_QWEN_SMALL_GUIDED_TEMPLATE_NO_HIS,
    _SOKOBAN_GUIDED_GRID_RESPONSE,
)
SOKOBAN_TEMPLATE = _sokoban_benchmark_prompt(
    SOKOBAN_QWEN_SMALL_GUIDED_TEMPLATE,
    _SOKOBAN_GUIDED_GRID_RESPONSE,
)
SOKOBAN_VISUAL_TEMPLATE = _sokoban_benchmark_prompt(
    SOKOBAN_QWEN_SMALL_GUIDED_VISUAL_TEMPLATE,
    _SOKOBAN_GUIDED_IMAGE_RESPONSE,
)

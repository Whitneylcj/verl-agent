...
class SokobanEnv(BaseDiscreteActionEnv, GymSokobanEnv):
    ...
    def __init__(self, mode, **kwargs):
        ...
        GymSokobanEnv.__init__(self, dim_room=kwargs.pop("dim_room", (6, 6)), max_steps=kwargs.pop("max_steps", 100), num_boxes=kwargs.pop("num_boxes", 3), **kwargs)
    ...
    def reset(self, seed=None):
        ...
        with NoLoggerWarnings():
            try:
                with set_seed(seed):
                    self.room_fixed, self.room_state, self.box_mapping, action_sequence = generate_room(dim=self.dim_room, num_steps=self.num_gen_steps, num_boxes=self.num_boxes, search_depth=self.search_depth)
            ...
            self._exact_reward_event_counts = {factor_id: 0 for factor_id in self.EXACT_REWARD_FACTOR_IDS}
    ...
    def step(self, action: int):
        ...
        self._record_exact_reward_events(
            ...
            current_boxes_on_target=int(self.boxes_on_target),
    ...
    def exact_reward_factor_state(self):
        """Return official cumulative reward events and their gym-sokoban weights."""

        return {
            "event_counts": dict(self._exact_reward_event_counts),
            "reward_weights": {
                "step_penalty": float(self.penalty_for_step),
                "box_on_target_reward": float(self.reward_box_on_target),
                "box_off_target_penalty": float(self.penalty_box_off_target),
                "all_boxes_on_target_reward": float(self.reward_finished),
            },
        }

    def _record_exact_reward_events(
        self,
        previous_boxes_on_target,
        current_boxes_on_target,
        completed,
        observed_reward,
    ):
        event_deltas = {
            "step_penalty": 1,
            "box_on_target_reward": int(current_boxes_on_target > previous_boxes_on_target),
            "box_off_target_penalty": int(current_boxes_on_target < previous_boxes_on_target),
            "all_boxes_on_target_reward": int(completed),
        }
        reward_weights = self.exact_reward_factor_state()["reward_weights"]
        reconstructed_reward = sum(reward_weights[factor_id] * event_deltas[factor_id] for factor_id in self.EXACT_REWARD_FACTOR_IDS)
        if not np.isclose(reconstructed_reward, observed_reward, rtol=0.0, atol=1e-12):
            raise RuntimeError(f"Sokoban official reward factors do not reconstruct step reward: factors={reconstructed_reward}, observed={observed_reward}")
        for factor_id, delta in event_deltas.items():
            self._exact_reward_event_counts[factor_id] += delta
    ...
    def copy(self):
        new_self = SokobanEnv(dim_room=self.dim_room, max_steps=self.max_steps, num_boxes=self.num_boxes, search_depth=self.search_depth)

...
def test_json_api_projection_compiles_one_fixed_shape_call():
    action = compile_json_api_action(
        '<think>inspect docs</think><action>{"app":"api_docs","api":"show_api_doc",'
        '"arguments":{"app_name":"spotify","include_private":false}}</action>'
    )
...
def test_json_api_projection_rejects_code_and_multiple_calls():
    invalid_actions = [
        '<think>x</think><action>{"app":"api_docs","api":"show_app_descriptions",'
        '"arguments":{}}</action><action>{"app":"supervisor","api":"complete_task",'
        '"arguments":{}}</action>',

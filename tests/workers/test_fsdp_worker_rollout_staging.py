...
def test_init_builds_rollout_before_sharding_manager_is_used() -> None:
    ...
    build_rollout_guard = next(
        ...
        and any(
            ...
            and call.func.attr == "_build_rollout"
...
def test_update_stages_rollout_weights_before_actor_offload() -> None:
    ...
    stage = next(
        node
        for node in calls
        if isinstance(node.func, ast.Attribute) and node.func.attr == "stage_updated_weights"
    )
    offload = next(
        node
        for node in calls
        if isinstance(node.func, ast.Name) and node.func.id == "offload_fsdp_model_to_cpu"
    )
